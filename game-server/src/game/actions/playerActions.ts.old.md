# playerActions.ts.old

**Location:** `game-server/src/game/actions/playerActions.ts.old`

---

## File Overview

This file is the legacy, monolithic implementation of all player actions for Darknet Duel. It contains the original logic for card cycling, playing, throwing, and turn management, as well as special effect handling and targeting validation. It has since been refactored into smaller, more maintainable modules, but is preserved here for reference and rollback.

- **Primary exports:** `cycleCardMove`, `playCardMove`, `throwCardMove`, `endTurnMove`, and utility functions
- **Purpose:** To provide a complete, in-place implementation of all player actions before modularization.
